...
class ftPwrDrive:
    ...
    def startMoving(self, motor, disable_on_stop=True):
        # start motor moving, disableOnStop disables the motor driver at the end of the movement
        self.__txt.i2c_write_buffer(self.__dev, struct.pack("<BBB", self.__CMD_STARTMOVING, motor, disable_on_stop),
                                    3)

    def startMovingAll(self, mask_motor, mask_disable_on_stop=0x0f):  # disable all on stop
        # same as StartMoving, but using masks
        self.__txt.i2c_write_buffer(self.__dev,
                                    struct.pack("<BBI", self.__CMD_STARTMOVINGALL, mask_motor, mask_disable_on_stop), 3)
    ...
    def homing(self, motor, max_distance, disable_on_stop=True):
        # homing of motor using end stop
        self.__txt.i2c_write_buffer(self.__dev,
                                    struct.pack("<BBIB", self.__CMD_HOMING, motor, max_distance, disable_on_stop),
                                    7)
